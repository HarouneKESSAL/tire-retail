                                    ...
                                    <ul>
                                        @php
                                            ...
                                            if (session()->has('coupon')) {
                                                $cartSubTotal -= session('coupon')['value'];
                                            }

                                            $totalAmount = $cartSubTotal + $shippingFee + $tpsTax + $tvqTax;
                                        @endphp

                                        <li class="order_subtotal" data-price="{{ $cartSubTotal }}">
                                            Sous-total du panier<span>{{ number_format($cartSubTotal, 2) }}€</span>
                                        </li>

                                        <li class="shipping">
                                            Coût de livraison
                                            @if(count(Helper::shipping()) > 0 && Helper::cartCount() > 0)
                                                <select name="shipping" class="nice-select" id="shipping-cost-selector">
                                                    <option value="0" data-price="0">Sélectionnez votre adresse</option>
                                        ...
                                        <li class="tax-tps">
                                            TPS (5%)<span>{{ number_format($tpsTax, 2) }}€</span>
                                        </li>

                                        <li class="tax-tvq">
                                            TVQ (9.975%)<span>{{ number_format($tvqTax, 2) }}€</span>
                                        </li>

                                        @if(session('coupon'))
                                            <li class="coupon_price" data-price="{{ session('coupon')['value'] }}">
                                                Vous économisez<span>{{ number_format(session('coupon')['value'], 2) }}€</span>
                                            </li>
                                        @endif

                                        <li class="last" id="order_total_price">
                                            Total<span id="total-amount">{{ number_format($totalAmount, 2) }}€</span>
                                        </li>
                                    </ul>

                                    <script>
                                        document.addEventListener('DOMContentLoaded', function() {
                                            document.getElementById('shipping-cost-selector').addEventListener('change', function() {
                                                const selectedOption = this.options[this.selectedIndex];
                                                const shippingCost = parseFloat(selectedOption.getAttribute('data-price')) || 0;
                                                const cartSubTotal = parseFloat(document.querySelector('.order_subtotal').getAttribute('data-price'));
                                                const tpsTax = cartSubTotal * 0.05;
                                                const tvqTax = cartSubTotal * 0.0975;
                                                const couponDiscount = parseFloat(document.querySelector('.coupon_price')?.getAttribute('data-price')) || 0;

                                                const totalAmount = cartSubTotal + shippingCost + tpsTax + tvqTax - couponDiscount;
